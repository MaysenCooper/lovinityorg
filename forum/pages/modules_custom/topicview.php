    ...
    public function run()
    {
        ...
        if (!$threaded) {
            ...
            foreach ($topic_info['posts'] as $array_id => $_postdetails) {
                ...
                // Load charms for inclusion on the post
		require_code('charms');
                $charms = $is_spacer_post ? null : get_charms__icons($_postdetails['poster']);
                ...
                if ($array_id == 0) {
                $rendered_post = do_template('CNS_TOPIC_POST_AD', array());
                $posts->attach($rendered_post);
                }
            }
        ...
        require_code('crypt');
        return internalise_own_screen($topic_tpl, 15, $topic_info['num_posts']);
    }
