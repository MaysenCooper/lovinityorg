...
class Module_filedump
{
    ...
    public function do_gui()
    {
        ...
        $place2 = explode("/", $this->place);
        $placeB = array_pop($place2);
		$placeB = array_pop($place2);
        $placeA = implode("/", $place2) . "/";
        if (has_privilege(get_member(), 'upload_filedump') && ($GLOBALS['FORUM_DRIVER']->is_super_admin(get_member()) || $GLOBALS['SITE_DB']->query_select_value_if_there('filedump', 'the_member', array('path' => $placeA, 'name' => $placeB)) === get_member() || $this->place === '/' . $GLOBALS['FORUM_DRIVER']->get_username(get_member()) . '/')) {
        ...
        $placeA = implode("/", $place2) . "/";
            $post_url = build_url(array('page' => '_SELF', 'type' => 'add', 'uploading' => 1), '_SELF');
        if ((get_option('is_on_folder_create') == '1') && (has_privilege(get_member(), 'upload_filedump')) && ($GLOBALS['FORUM_DRIVER']->is_super_admin(get_member()) || $GLOBALS['SITE_DB']->query_select_value_if_there('filedump', 'the_member', array('path' => $placeA, 'name' => $placeB)) === get_member() || $this->place === '/' . $GLOBALS['FORUM_DRIVER']->get_username(get_member()) . '/')) {
    ...
    public function do_mass()
    {
        ...
        foreach ($files as $_file) {
            ...
            if (((!is_null($owner)) && ($owner == get_member())) || ((has_privilege(get_member(), 'delete_anything_filedump')) || $owner == get_member() || $place === '/' . $GLOBALS['FORUM_DRIVER']->get_username(get_member()) . '/')) {
    ...
    public function do_add_folder()
    {
		...
		$placeB = array_pop($place2);
        $placeA = implode("/", $place2) . "/";
        if (!has_privilege(get_member(), 'upload_filedump') && ($GLOBALS['FORUM_DRIVER']->is_super_admin(get_member()) || $GLOBALS['SITE_DB']->query_select_value_if_there('filedump', 'the_member', array('path' => $placeA, 'name' => $placeB)) === get_member() || $place === '/' . $GLOBALS['FORUM_DRIVER']->get_username(get_member()) . '/')) {
            access_denied('I_ERROR');
        }
