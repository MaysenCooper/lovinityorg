...
function _log_hack_attack_and_exit($reason, $reason_param_a = '', $reason_param_b = '', $silent = false, $instant_ban = false)
{
	...
	$postparam = array();
	$embedfields = array();
	$embedfields[0] = array('name' => 'Hack attempt', 'value' => $reason);
	$embedfields[1] = array('name' => 'IP Address', 'value' => $ip);
	$embeds = array();
	$embeds[0] = array('title' => 'HACK ATTACK DETECTED', 'color' => '16711680', 'thumbnail' => $thumbfields, 'fields' => $embedfields);
...
function add_ip_ban($ip, $descrip = '', $ban_until = null, $ban_positive = true)
{
    ...
    $alreadybanned = $GLOBALS['SITE_DB']->query_select_value_if_there('banned_ip', 'ip', array('ip' => $ip, 'i_ban_positive' => 1), ' AND (i_ban_until IS NULL OR i_ban_until > ' . time() . ')');
    ...
    if ($ban_positive && $alreadybanned === null)
    {
	...
	$postparam = array();
	$embedfields = array();
	$embedfields[0] = array('name' => 'IP banned', 'value' => $ip);
	$embedfields[1] = array('name' => 'Reason for ban', 'value' => $descrip);
	$embeds = array();
	$embeds[0] = array('title' => 'An IP ban has been placed', 'color' => '255', 'thumbnail' => $thumbfields, 'fields' => $embedfields);
	$postparam['embeds'] = $embeds;
	$postparam = json_encode($postparam);
	http_download_file('https://discordapp.com/api/webhooks/287989749447393282/XJoqmdYbDU79AACabGdhSMdsXLfu0-jQSwO20x9ynZmRYdRhQLpjokfpxnkb4r1vkZKr', null, false, true, 'Composr', array($postparam), null, null, null, null, null, null, null, 6.0, true, null, null, null, 'application/json');
...
function remove_ip_ban($ip)
{
	...
	$postparam = array();
	$embedfields = array();
	$embedfields[0] = array('name' => 'IP unbanned', 'value' => $ip);
	$embeds = array();
	$embeds[0] = array('title' => 'An IP ban has been removed', 'color' => '255', 'thumbnail' => $thumbfields, 'fields' => $embedfields);
	$postparam['embeds'] = $embeds;
	$postparam = json_encode($postparam);
	http_download_file('https://discordapp.com/api/webhooks/287989749447393282/XJoqmdYbDU79AACabGdhSMdsXLfu0-jQSwO20x9ynZmRYdRhQLpjokfpxnkb4r1vkZKr', null, false, true, 'Composr', array($postparam), null, null, null, null, null, null, null, 6.0, true, null, null, null, 'application/json');
...
function _fatal_exit($text, $return = false)
{
	...
	require_code('addons');
	$thumbfields = array();
	$addonthumb = find_addon_icon('errorlog');
	$params = array($addonthumb,'36x36','addon_icon_normalise','','','pad','both','#FFFFFF00');
	$ensurethumb = _symbol_thumbnail($params);
	$thumbfields['url'] = $ensurethumb;
	$html = (is_object($text) ? $text->evaluate() : $text);
	$html = html_to_comcode($html);
	$html = comcode_to_clean_text($html);
	$postparam = array();
	$embedfields = array();
	$embedfields[0] = array('name' => 'Error', 'value' => $html);
	$embeds = array();
	$embeds[0] = array('title' => 'A fatal exit has occurred', 'color' => '16744448', 'thumbnail' => $thumbfields, 'fields' => $embedfields);
	$postparam['embeds'] = $embeds;
	$postparam['content'] = '<@&279071064783585281>';
	$postparam = json_encode($postparam);
	http_download_file('https://discordapp.com/api/webhooks/287989749447393282/XJoqmdYbDU79AACabGdhSMdsXLfu0-jQSwO20x9ynZmRYdRhQLpjokfpxnkb4r1vkZKr', null, false, true, 'Composr', array($postparam), null, null, null, null, null, null, null, 6.0, true, null, null, null, 'application/json');
