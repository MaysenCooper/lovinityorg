...
function _password_censor($text, $scan_type = 1, $explicit_only = false)
{
    ...
    if (($explicit_only) || (strpos($text, '[self_destruct') !== false) || (strpos($text, '[encrypt') !== false) || (strpos($text, '[pencrypt') !== false)) { // Explicit control, Comcode writer knows what they're doing
        ...
        $matches = array();
        $num_matches = preg_match_all('#\[pencrypt[^\]]*\](.*)\[/pencrypt\]#Us', $text, $matches);
        for ($i = 0; $i < $num_matches; $i++) {
            if ($scan_type != PASSWORD_CENSOR__PRE_SCAN) {
                $text = str_replace($matches[0][$i], '(encrypted)', $text);
            } else {
                require_code('encryption');
                if (is_encryption_enabled()) {
                    $text = str_replace($matches[0][$i], '[pencrypt]' . encrypt_data($matches[1][$i], true) . '[/pencrypt]', $text);
                } else {
                    $text = str_replace($matches[0][$i], '(encryption not available, cannot save)', $text);
                }
            }
