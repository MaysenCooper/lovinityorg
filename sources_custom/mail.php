...
function mail_wrap($subject_line, $message_raw, $to_email = null, $to_name = null, $from_email = '', $from_name = '', $priority = 3, $attachments = null, $no_cc = false, $as = null, $as_admin = false, $in_html = false, $coming_out_of_queue = false, $mail_template = 'MAIL', $bypass_queue = null, $extra_cc_addresses = null, $extra_bcc_addresses = null, $require_recipient_valid_since = null)
{
    ...
    // Do not send emails below priority 1 to banned or suspended users
    ...
    // Check each user's privacy email setting set by staff (field 70)
    foreach ($to_email as $key => $email)
    {
    	$prioritylimit = $GLOBALS['SITE_DB']->query_select_value_if_there('f_member_custom_fields', 'field_70', array('mf_member_id' => $GLOBALS['FORUM_DRIVER']->get_member_from_email_address($email)));
    	if (!is_null($prioritylimit) && $priority > $prioritylimit)
    		unset($to_email[$key]);
    }
