...
function activities_addon_syndicate_described_activity($a_language_string_code = '', $a_label_1 = '', $a_label_2 = '', $a_label_3 = '', $a_page_link_1 = '', $a_page_link_2 = '', $a_page_link_3 = '', $a_addon = '', $a_is_public = 1, $a_member_id = null, $sitewide_too = false, $a_also_involving = null)
{
    ...
    if ((!array_key_exists(0, $test)) || ($test[0] != $go) || (running_script('execute_temp')) || ($GLOBALS['SEMI_DEV_MODE'])) {
		...
		if ($a_language_string_code === 'RAW_DUMP')
			log_it('STATUS_UPDATE',$stored_id);
        // External places
        ...
        $thelink = page_link_to_tempcode($a_page_link_1);
        if ($thelink === null || $thelink === '')
			$thelink = page_link_to_tempcode(':start');
        $arrowchat_notification = $GLOBALS['SITE_DB']->query('CALL NewActivity(\'' . db_escape_string(strip_html($message->evaluate())) . '\',' . $a_member_id . ',\'' . $GLOBALS['FORUM_DRIVER']->get_username($a_member_id) . '\',' . time() . ',\'' . db_escape_string($thelink->evaluate()) . '\')');

	//require_code('activities');
	//$description = render_activity($row, true, true);
	...
	require_code('addons');
	$thumbfields = array();
	$addonthumb = ($a_addon == '') ? find_addon_icon('activity_feed') : find_addon_icon($a_addon);
	...
	$html = $message->evaluate();
	$html = html_to_comcode($html);
	$html = comcode_to_clean_text($html);
	$html2 = $thelink->evaluate();
	$html2 = html_to_comcode($html2);
	$html2 = comcode_to_clean_text($html2);
	$postparam = array();
	$embedfields = array();
	$embedfields[0] = array('name' => 'Username', 'value' => $GLOBALS['FORUM_DRIVER']->get_username($a_member_id));
	$embedfields[1] = array('name' => 'Activity', 'value' => (strlen($html) > 253) ? substr($html,0,253).'...' : $html);
	$embedfields[2] = array('name' => 'Link', 'value' => $html2);
	//if ($description !== null)
	//$embedfields[3] = array('name' => 'Description', 'value' => (strlen($description) > 253) ? substr($description,0,253).'...' : $description);
	$embeds = array();
	$embeds[0] = array('title' => 'Activity on lovinity.org', 'color' => '16744703', 'thumbnail' => $thumbfields, 'fields' => $embedfields);
