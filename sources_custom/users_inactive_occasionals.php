...
function create_session($member, $session_confirmed = 0, $invisible = false)
{
    ...
    $demerits = $GLOBALS['SITE_DB']->query_select_value_if_there('f_member_custom_fields', 'field_40', array('mf_member_id' => $member));
    ...
    if ($demerits < 0)
    $demerits = 0;
    $GLOBALS['FORUM_DB']->query_update('f_member_custom_fields', array('field_40' => $demerits), array('mf_member_id' => $member), '', 1);
		require_code('cns_general');
		$stats = cns_get_forums_stats();
		$nummembers = integer_format($stats['num_members']) - 1;
    $starstat = $GLOBALS['SITE_DB']->query_select_value_if_there('f_member_custom_fields', 'field_44', array('mf_member_id' => $member));
    $stockindex = float_format(get_value('current_activity_index', 0));
    $indextobeat = float_format((get_value('max_star_index', 0))*0.2);
    $maxdividend = (($stockindex / ($nummembers * 0.2)) * 5);
                require_code('points');
		require_code('points2');
		If ($demerits < 1 && $starstat >= $indextobeat)
			system_gift_transfer('Daily dividend for being among most active users',intval($maxdividend*($starstat/($indextobeat*5))),$member);
