 ...
 */
function render_activity($row, $use_inside_cms = true, $description_only = false)
{
    $guest_id = $GLOBALS['FORUM_DRIVER']->get_guest_id();
    ...
    $_attributes = array();
    $hash = '';
    if ($row['a_page_link_1'] != '')
		list(,$attributes,$hash) = page_link_decode($row['a_page_link_1']);
	if ($hash !== '')
	{
		$_attributes = explode('_',$hash);
		$attributes['type'] = $_attributes[0];
		$attributes['id'] = $_attributes[1];
	}
    for ($i = 1; $i <= 3; $i++) {
    ...
    //Attach a render box if possible
    if (!empty($attributes) && array_key_exists('type',$attributes) && array_key_exists('id',$attributes) && $attributes['type'] !== '' && $attributes['id'] !== '' && $use_inside_cms)
    {
		require_code('content');
		require_code('urls');
		$theattribute = 'type';
		$ob=get_content_object($attributes['type']); 
		if ($ob === null || $ob === '') {
		if ($attributes['page'] === 'catalogues' && $attributes['type'] === 'entry')
		{
			$attributes['type'] = 'catalogue_entry';
			$ob=get_content_object($attributes['type']); 
		} else if ($attributes['page'] === 'news' && $attributes['type'] === 'view')
		{
			$attributes['type'] = 'news';
			$ob=get_content_object($attributes['type']);
		} else if ($attributes['page'] === 'polls' && $attributes['type'] === 'view')
		{
			$attributes['type'] = 'poll';
			$ob=get_content_object($attributes['type']);
		} else if ($attributes['page'] === 'forumview' && $attributes['type'] === 'browse')
		{
			$attributes['type'] = 'forum';
			$ob=get_content_object($attributes['type']);
		} else if ($attributes['page'] === 'calendar' && $attributes['type'] === 'view')
		{
			$attributes['type'] = 'event';
			$ob=get_content_object($attributes['type']);
		} else if ($attributes['page'] === 'downloads' && $attributes['type'] === 'entry')
		{
			$attributes['type'] = 'download';
			$ob=get_content_object($attributes['type']);
		} else if ($attributes['page'] === 'quiz' && $attributes['type'] === 'do')
		{
		...
		{
		$theid = null;
		//$theid = find_id_via_url_moniker($attributes[$theattribute], $attributes['id']);
		if ($theid !== null && $theid !== '')
			$attributes['id'] = $theid;
		list($content_title, $poster_id, $cma_info, $content_row, $content_url) = content_get_details($attributes[$theattribute], $attributes['id']);
		$message->attach(($ob->run($content_row, get_module_zone($cma_info['module'])))->evaluate());
		}
	}
    if (!empty($row['a_render_html']) && $use_inside_cms)
        $message->attach($row['a_render_html']);
    
if ($description_only)
{

$info = $ob->info();
if ($info['description_field'] !== null)
{
$description = $GLOBALS['SITE_DB']->query_select_value_if_there($info['table'], $info['description_field'], get_content_where_for_str_id($attributes['id'], $info));
return $description;
}
return null;
}
