...
// demerits
$demerits = '';
if ((has_privilege($member_id_viewing, 'warn_members')) || $member_id_of == $member_id_viewing) {
$demerits = 0;
$demerits = $GLOBALS['SITE_DB']->query_select_value_if_there('f_member_custom_fields', 'field_40', array('mf_member_id' => $member_id_of));
if ($demerits == null || $demerits == '')
$demerits = 0;
}
		
		// popularity
			$maxpopularity = intval(get_value('max_star_index', 0));
			$popularity = $GLOBALS['SITE_DB']->query_select_value_if_there('f_member_custom_fields', 'field_44', array('mf_member_id' => $member_id_of));
			$multiplier = (($popularity / $maxpopularity)*100);
			if ($multiplier > 100)
			$multiplier = 100;
	    ...
	    'DEMERIT_INT' => strval($demerits),
	    'POP_INT' => strval($popularity),
	    'POP_MULT' => strval($multiplier),
	    ...
	    'ONLINE_STATE' => strval($onlinestate),
        ) + $fields_map);
