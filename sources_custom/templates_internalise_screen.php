 ...
 */
function internalise_own_screen($screen_content, $refresh_time = null, $refresh_if_changed = null, $feedback_id = null)
{
    if (!has_js()) {
    ...
    foreach ($_GET as $key => $param) {
        ...
        if (($key == 'page') && !is_null($feedback_id)) {
	    $params .= (($params == '') ? '?' : '&') . 'page=' . urlencode($param);
            $params .= (($params == '') ? '?' : '&') . 'page2=feedback';
            $params .= (($params == '') ? '?' : '&') . 'id2=' . $feedback_id;
            continue;
        }
        if ($key == 'page') {
	    $params .= (($params == '') ? '?' : '&') . 'page=' . urlencode($param);
	    $params .= (($params == '') ? '?' : '&') . 'page2=' . urlencode($param);
	    continue;
	}
