		...
		{+START,IF_NON_EMPTY,{POSTER}}
			...
			<div>
				...
				{+START,IF_NON_EMPTY,{RANK_IMAGES}}<div class="cns_topic_poster_rank_images">{RANK_IMAGES}</div>{+END}
                                <div style="text-align: center;"><canvas id="can_p_{ID*}" width="75" height="75" /></div>
                                {CHARMS}
			</div>
		{+END}
	</div>
...
{+START,IF_NON_EMPTY,{POSTER_ID}}
<script>		
add_event_listener_abstract(window,'load',function() {
        var pie_{ID*} = new RGraph.Pie({
            id: 'can_p_{ID*}',
            data: [{$CPF_VALUE,65,{POSTER_ID}},{$CPF_VALUE,66,{POSTER_ID}},{$CPF_VALUE,67,{POSTER_ID}},{$CPF_VALUE,68,{POSTER_ID}}],
            options: {
                radius: 35,
                title: 'Credibility:\n{$CPF_VALUE,64,{POSTER_ID}}',
                titleSize: 7,
                titleY: 37,
                colors: ['red','coral','lightgreen','green'],
                tooltips: ['Bad credibility - Staff (-{$CPF_VALUE,65,{POSTER_ID}})','Bad credibility - Users (-{$CPF_VALUE,66,{POSTER_ID}})','Good credibility - Users ({$CPF_VALUE,67,{POSTER_ID}})','Good Credibility - Staff ({$CPF_VALUE,68,{POSTER_ID}})'],
                textAccessible: true,
                shadow: false,
            }
        }).draw();
