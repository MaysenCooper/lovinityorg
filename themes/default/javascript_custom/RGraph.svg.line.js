...
(function (win, doc, undefined)
{
    ...
    RG.SVG.Line = function (conf)
    {
        ...
        this.data            = typeof conf.data[0] === 'number' ? [conf.data] : conf.data;
        ...
        this.properties =
        {
            ...
            textColor: 'black',
            textFont: 'Arial,Verdana,Segoe UI,sans-serif',
            textSize: 12,
            textBold: false,
            textItalic: false,
        ...
        */
        ...
        this.drawLine = function (data, index)
        {
            ...
            for (var i=0; i<coords.length; ++i) {
                if (i === 0) {
                    path.push('M{1} {2}'.format(coords[i][0], coords[i][1]));
                } else {
                    path.push('L{1} {2}'.format(coords[i][0], coords[i][1]));
                }
            }
            ...
            if (prop.filled === true || (typeof prop.filled === 'object' && prop.filled[index]) ) {
                ...
                fillPath.push('L{1} {2}'.format(
                    ...
                    index > 0 && prop.filledAccumulative ? (prop.spline ? this.coordsSpline[index - 1][this.coordsSpline[index - 1].length - 1][1] : this.coords[index - 1][this.coords[index - 1].length - 1][1]) : this.getYCoord(prop.yaxisMin) + (prop.xaxis ? 0 : 1)
                ));
                ...
                if (index > 0&& prop.filledAccumulative) {
                    ...
                    fillPath.push('L{1} {2}'.format(
                        ...
                        this.getYCoord(prop.yaxisMin) + (prop.xaxis ? 0 : 1)
                    ));
                }
        ...
        this.parseColors = function () 
        {
            if (!Object.keys(this.originalColors).length) {
                this.originalColors = {
                    ...
                    yaxisColor:          RG.SVG.arrayClone(prop.yaxisColor),
                    xaxisColor:          RG.SVG.arrayClone(prop.xaxisColor),
                    textColor:           RG.SVG.arrayClone(prop.textColor),
                    highlightStroke:     RG.SVG.arrayClone(prop.highlightStroke),
                    highlightFill:       RG.SVG.arrayClone(prop.highlightFill),
                    titleColor:          RG.SVG.arrayClone(prop.titleColor)
                }
            }
            ...
            prop.yaxisColor          = RG.SVG.parseColorLinear({object: this, color: prop.yaxisColor});
            prop.xaxisColor          = RG.SVG.parseColorLinear({object: this, color: prop.xaxisColor});
            prop.textColor           = RG.SVG.parseColorLinear({object: this, color: prop.textColor});
            prop.titleColor          = RG.SVG.parseColorLinear({object: this, color: prop.titleColor});
            prop.highlightStroke     = RG.SVG.parseColorLinear({object: this, color: prop.highlightStroke});
