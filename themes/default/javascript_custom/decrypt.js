...
{
	console.log("Initiated function decrypt_it"); 
	if (document.getElementById('decryption_overlay')) return;
	console.log("did not detect presence of decryption overlay"); 
	if (passcode === null)
	{
	console.log("no passcode provided"); 
	var needs_passcode=load_snippet('decrypt','action=needspasscode&data=' + get_inner_html(document.getElementById('decrypt_' + encrypted_id)));
	console.log("snippet to check if passcode needed loaded. Response: " + needs_passcode); 
	if (needs_passcode == 1)
	{
		console.log("passcode needed. Building form."); 
		...
		form.action='#';
		form.onsubmit = 'return decrypt_it2(this);';
		form.method='post';
	...
	{
		console.log("passcode not needed. Decrypting immediately."); 
		set_inner_html(document.getElementById('decrypt_' + encrypted_id),load_snippet('decrypt','action=decrypt&data=' + get_inner_html(document.getElementById('decrypt_' + encrypted_id))));
		set_inner_html(document.getElementById('decryptbutton_' + encrypted_id),'');
		console.log("Decryption complete."); 
	}
	} else {
		console.log("Passcode needed and provided. Decrypting."); 
		set_inner_html(document.getElementById('decrypt_' + encrypted_id),load_snippet('decrypt','action=decrypt&data=' + get_inner_html(document.getElementById('decrypt_' + encrypted_id))));
		set_inner_html(document.getElementById('decryptbutton_' + encrypted_id),'');
		console.log("Decryption complete."); 
	}
...
{
	console.log("Decrypt_it2 function called. Executing decrypt_it from form. Encrypted_id: " + form.encrypted_id + " , password: " + form.password); 
	decrypt_it(form.encrypted_id, form.password);
	return;
}