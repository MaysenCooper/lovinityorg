...
set_inner_html(document.getElementById('toggle_wysiwyg_'+id),'<img src="https://cdn1.lovinity.org/themes/default/images/icons/16x16/editor/wysiwyg_on.png" srcset="https://cdn1.lovinity.org/themes/default/images/icons/16x16/editor/wysiwyg_on.png 2x" alt="Enable WYSIWYG editing" title="Enable WYSIWYG editing" class="vertical_alignment" />');try
{window.wysiwyg_editors[id].destroy();}
catch(e){}}}}
...
set_inner_html(document.getElementById('toggle_wysiwyg_'+id),'<img src="https://cdn1.lovinity.org/themes/default/images/icons/16x16/editor/wysiwyg_off.png" srcset="https://cdn1.lovinity.org/themes/default/images/icons/32x32/editor/wysiwyg_off.png 2x" alt="Disable WYSIWYG editing" title="Disable WYSIWYG editing" class="vertical_alignment" />');window.wysiwyg_original_comcode[id]=e.value;if(!ajax_copy)
...
toolbar.push(['composr_block','composr_comcode',precision_editing?'composr_page':null,'composr_quote',precision_editing?'composr_box':null,'composr_code']);var editor_settings={skin:'kama',enterMode:window.CKEDITOR.ENTER_BR,uiColor:wysiwyg_color,ocpTheme:'admin',fontSize_sizes:'0.6em;0.85em;1em;1.1em;1.2em;1.3em;1.4em;1.5em;1.6em;1.7em;1.8em;2em',removePlugins:'',extraPlugins:'showcomcodeblocks,imagepaste,spellchecktoggle'+(use_composr_toolbar?',composr':''),customConfig:'',bodyId:'wysiwyg_editor',baseHref:get_base_url()+'/',linkShowAdvancedTab:true,imageShowAdvancedTab:true,imageShowLinkTab:true,imageShowSizing:true,autoUpdateElement:true,contentsCss:page_stylesheets,font_names:'banten-unfamous-2; casper-comics; comingsoon; conthrax-sb; ldfcomicsans; nesobritescbk-regular; sf-orson-casual-light; yonder-brk; Arial; Courier; Georgia; Impact; Lucida Sans; Times; Trebuchet; Verdana; Tahoma; Geneva; Helvetica;',cssStatic:css,startupOutlineBlocks:true,language:(window.cms_lang)?cms_lang.toLowerCase():'en',emailProtection:false,resize_enabled:true,width:(find_width(element)-15),height:(window.location.href.indexOf('cms_comcode_pages')==-1)?250:500,toolbar:toolbar,allowedContent:true,browserContextMenuOnCtrl:true,comcodeXMLBlockTags:'overlay,section,big_tab,tabs,tab,carousel,media_set,staff_note,box,codebox,no_parse,code,hide,quote,moderator_message,moderator_closed,moderator_moved,moderator_featured,moderator_inactive_account,encrypt,pencrypt',comcodeXMLInlineTags:'tooltip,if_in_group,img,concept,ticker,pulse,require_css,require_javascript',magicline_everywhere:true};if(typeof window.CKEDITOR.instances[element.id]!='undefined'&&window.CKEDITOR.instances[element.id])
...
window.lang_PREFER_CMS_ATTACHMENTS='You will be taken to add a raw image. Note that the website has an attachments feature (underneath the editor, the Attachments section). Attachments come with an easy upload and sophisticated document display settings.';window.lang_SPELLCHECKER_ENABLED='Enabled spellchecker. Disabled table and list management.';window.lang_SPELLCHECKER_DISABLED='Disabled spellchecker. Enabled table and list management.';window.lang_SPELLCHECKER_TOGGLE='Toggle spellchecking';window.lang_SPELLCHECKER_LABEL='Spell checker';window.lang_NO_IMAGE_PASTE_SAFARI='Sorry, images cannot be pasted in Safari';editor.on('change',function(event){if(typeof element.externalOnKeyPress!='undefined')
{element.value=editor.getData();element.externalOnKeyPress(event,element);}});editor.on('instanceReady',function(event){editor.setReadOnly(false);if(typeof window.set_up_comcode_autocomplete!='undefined')
...
{if(event.pageX)eventCopy.pageX=3000;if(event.clientX)eventCopy.clientX=3000;if(event.pageY)eventCopy.pageY=3000;if(event.clientY)eventCopy.clientY=3000;if(typeof window.activate_tooltip!='undefined')
...
{if(!authorised)
{var url='https://www.lovinity.org/data/upload_syndication_auth.php?hook='+window.encodeURIComponent(hook)+'&name='+window.encodeURIComponent(name)+keep_stub();faux_open(url,null,'width=960;height=500','_top');if(!pre_disabled)
