...
{window.notifications_already_presented={};window.NOTIFICATION_POLL_FREQUENCY=30;window.notifications_time_barrier=null;}
...
{var url='https://www.lovinity.org/data/notifications.php?type=poller&type=mark_all_read';if(typeof window.max_notifications_to_show!='undefined')url+='&max='+window.max_notifications_to_show;url+='&time_barrier='+window.encodeURIComponent(window.notifications_time_barrier);url+='&forced_update=1';url+=keep_stub();do_ajax_request(url,window._poll_for_notifications);_toggle_messaging_box(event,'web_notifications',true);return false;}
...
var url='https://www.lovinity.org/data/notifications.php?type=poller&type=poller';if(typeof window.max_notifications_to_show!='undefined')url+='&max='+window.max_notifications_to_show;url+='&time_barrier='+window.encodeURIComponent(window.notifications_time_barrier);if(forced_update)url+='&forced_update=1';url+=keep_stub();do_ajax_request(url,window._poll_for_notifications);}
...
{unread=raw_ajax_result.getElementsByTagName('unread_web_notifications');set_inner_html(spot,get_inner_html(display[0]));set_inner_html(button.childNodes[0],get_inner_html(unread[0]));set_inner_html(document.getElementById('web_notifications_num'),get_inner_html(unread[0]));button.className='count_'+get_inner_html(unread[0]);}}
...
{unread=raw_ajax_result.getElementsByTagName('unread_pts');set_inner_html(spot,get_inner_html(display[0]));set_inner_html(button.childNodes[0],get_inner_html(unread[0]));set_inner_html(document.getElementById('pts_num'),get_inner_html(unread[0]));button.className='count_'+get_inner_html(unread[0]);}}}
...
{var go_func=function(){var sound_url='data/sounds/gamechime.mp3';var base_url=((sound_url.indexOf('data_custom')==-1)&&(sound_url.indexOf('uploads/')==-1))?'':'https://www.lovinity.org';var sound_object=window.soundManager.createSound({url:base_url+'/'+sound_url});if(sound_object)sound_object.play();};if(!window.soundManager.setupOptions.url)
