		...
		<div class="cns_information_bar float_surrounder">
			...
			<nav style="min-height: {$MAX,100,{MAX_AVATAR_HEIGHT|}}px" class="cns_member_column cns_member_column_d">
				...
				<ul class="actions_list">
					...
					<li><a{+START,IF,{$DESKTOP}} onclick="return open_link_as_overlay(this);"{+END} href="{NEW_POSTS_URL*}">{!POSTS_SINCE}</a></li>
					<li><a{+START,IF,{$DESKTOP}} onclick="return open_link_as_overlay(this);"{+END} href="{UNREAD_TOPICS_URL*}">{!TOPICS_UNREAD}</a></li>
					<li><a{+START,IF,{$DESKTOP}} onclick="return open_link_as_overlay(this);"{+END} href="{RECENTLY_READ_URL*}">{!RECENTLY_READ}</a></li>
					<li><a{+START,IF,{$DESKTOP}} onclick="return open_link_as_overlay(this);"{+END} href="{INLINE_PERSONAL_POSTS_URL*}">{!INLINE_PERSONAL_POSTS}</a></li>
					<li><a{+START,IF,{$DESKTOP}} onclick="return open_link_as_overlay(this);"{+END} href="{UNANSWERED_TOPICS_URL*}">{!UNANSWERED_TOPICS}</a></li>
					<li><a{+START,IF,{$DESKTOP}} onclick="return open_link_as_overlay(this);"{+END} href="{INVOLVED_TOPICS_URL*}">{!INVOLVED_TOPICS}</a></li>
				</ul>
			</nav>
		</div>
	</div>
