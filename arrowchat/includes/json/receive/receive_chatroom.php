<?php
	...
	*/
	...
	$chatroomtabs		= get_var('chatroomtabs');
	...
	if (logged_in($userid)) 
	{
		...
		if (!empty($chatroomtabs))
		{
			$unfocused_tabs_array = explode(",", $chatroomtabs);
			...
			foreach ($unfocused_tabs_array as $key => $value)
			{
				if (is_numeric($value))
				{
					$db->execute("
						...
						VALUES ('" . $db->escape_string($userid) . "', '" . $db->escape_string($value) . "', '" . $time . "') 
						ON DUPLICATE KEY 
							UPDATE chatroom_id = '" . $db->escape_string($value) . "', session_time = '" . $time . "'
					");
				}
			}
		}
