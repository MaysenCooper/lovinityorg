			...
			if (!$messages_are_limited)
			{
				if (empty($row['silence_time']) OR $row['silence_time'] + $row['silence_length'] < time())
				{
					...
					if ($chatroomid == 1)
					{
						...
						http_download_file('https://discordapp.com/api/webhooks/284190422861873152/m44ZPMM6Y8ding09L-RD9dQ3Fp-5swTQhJv65dS22IOklML_t1vg_HTP0EWW9sb2qW7u', null, false, true, 'Composr', array('content' => $db->escape_string($username) . ' says: ' . $db->escape_string($s_message)));
					}
