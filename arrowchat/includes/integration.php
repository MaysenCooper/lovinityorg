<?php
	...
	*/
	...
	require_once('/var/www/vhosts/lovinity.org/httpdocs/data_custom/arrowchat_integration.php');
	error_reporting(0);
	...
	function get_online_list($userid, $time) 
	{
		...
		$sql = ("
			...
			JOIN arrowchat_status 
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = arrowchat_status.userid 
			WHERE (arrowchat_status.session_time > (" . time() . " - " . $online_timeout . " - 60) OR arrowchat_status.userid = 3)
	...
	{
		return $GLOBALS['FORUM_DRIVER']->member_profile_url($user_id);
	}
	...
	{
   global $base_url;
$file1 = 'https://lovinity.org/' . $image;
$file_headers1 = @get_headers($file1);
$file_headers2 = @get_headers($image);
...
return $image;
    ...
    $result = $db->execute("
      SELECT gm_group_id
      FROM " . TABLE_PREFIX . "f_group_members
      WHERE gm_member_id = '" . $db->escape_string($userid) . "' AND gm_validated = '1'
    ");
      ...
      while ($row = $db->fetch_array($result))
      {
        ...
        $group_ids[] = $row['gm_group_id'];
      }
  
    
      $result = $db->execute("
      SELECT m_primary_group
      FROM " . TABLE_PREFIX . "f_members
      WHERE id = '" . $db->escape_string($userid) . "'
    ");
      ...
      {
        // Fill the group IDs into an array
        $group_ids2[] = $row['m_primary_group'];
      }
    
    // Delete following line after customizing this function
    return array_merge($group_ids,$group_ids2);
	}
    ...
    {   
      while ($row = $db->fetch_array($result))
      {
        ...
        $groups[] = array($row['id'], $row['g_name']);
      }
      return $groups;
    }
    else
