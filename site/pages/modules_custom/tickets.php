...
class Module_tickets
{
	...
	require_code('addons');
	$thumbfields = array();
	$addonthumb = find_addon_icon('tickets');
	...
	$postparam = array();
	$embedfields = array();
	$embedfields[0] = array('name' => 'Ticket Title', 'value' => $_title);
	$embedfields[1] = array('name' => 'Ticket URL', 'value' => $home_url);
	$embedfields[2] = array('name' => 'Further Instructions', 'value' => 'Please react to this message with a thumbs up to indicate you will handle this. Please do not handle tickets you are not assigned to unless no one has been assigned to it.');
	$embeds = array();
	$embeds[0] = array('title' => 'Support Ticket Activity', 'color' => '65535', 'thumbnail' => $thumbfields, 'fields' => $embedfields);
	...
	$postparam = array();
	$embedfields = array();
	$embedfields[0] = array('name' => 'Ticket Title', 'value' => $ticket_title);
	$embedfields[1] = array('name' => 'Ticket URL', 'value' => $home_url);
	$embedfields[2] = array('name' => 'Username of Staff ticket was assigned to', 'value' => $GLOBALS['FORUM_DRIVER']->get_username($member_id));
	$embedfields[3] = array('name' => 'Further Instructions', 'value' => 'If you have been assigned to this ticket, please complete it ASAP.');
	$embeds = array();
	$embeds[0] = array('title' => 'A Support Ticket has been assigned to staff', 'color' => '65535', 'thumbnail' => $thumbfields, 'fields' => $embedfields);
	$postparam['embeds'] = $embeds;
	$postparam = json_encode($postparam);
	http_download_file('https://discordapp.com/api/webhooks/276127156701102083/AC3U7mWGK04oYIQ0sPITYD7QY60VAMWPMBUU_KxAhdW2SCnFbtw228CHpP4MP8EGi5p_', null, false, true, 'Composr', array($postparam), null, null, null, null, null, null, null, 6.0, true, null, null, null, 'application/json');
